...
def determine_initial_direction(train, location_object, initial_track_id):
    ...
    if initial_track_id is None:
        logging.info("Initial track ID is None")
        return None

    track_parts = location_object.get("trackParts", [])
    logging.debug(initial_track_id)
    for tp in track_parts:
        if tp["id"] == initial_track_id:
            logging.debug("found")
            if tp.get("sawMovementAllowed", False):
                logging.info("Track is turnable, defaulting to bside")
            ...
            logging.debug(f"aSide={tp.get('aSide')} bSide={tp.get('bSide')}")
            logging.debug(
                f"has_aside_connection={has_aside_connection} "
                f"has_bside_connection={has_bside_connection}"
            )
...
def create_instance_from_scenario(path_to_folder=None, scenario_file=None, location_file=None, output_file=None, domain_file=None, coupling_mode="implicit_free_uncoupling", subproblem="combined"):
   ...
   # If location not specified use default
    ...
    # logging.info(problem.actions)
    writer = PDDLWriter(problem)
    writer.write_problem(output_file)

    # Print the output file of the domain
    logging.info(f"Problem file written to: {output_file}")
    ...
    logging.info(f"Domain file written to: {domain_file}")
