...
def plan_with_julia(pddl_path, timeout=300):
    """Disabled: all planning must use the local ENHSP jar."""
    raise RuntimeError("SymbolicPlanners.jl backend is disabled; use local ENHSP jar")
...
def process_scenario(scenario_path, idx, total, run_id, use_examples, planner):
    ...
    try:
        ...
        if planner != "enhsp":
            raise ValueError("Only the local ENHSP jar planner is supported")
