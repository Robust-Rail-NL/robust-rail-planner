...
def _relevant_corridor_nodes(scenario_object, location_object, known_track_ids, coupling_candidate_track_ids, service_track_ids=(), expand_hops=CORRIDOR_EXPAND_HOPS):
    ...
    # A service requirement is unreachable when corridor pruning omits every matching
    # facility. Include each task-bearing train's path to the available service tracks;
    # the existing request paths then connect that service detour back to its destination.
    for train, start_id in trains_with_starts:
        if not any(member.get("tasks", []) for member in train.get("members", [])):
            continue
        for service_id in service_track_ids:
            add_path(start_id, str(service_id))
...
def create_instance_from_scenario(path_to_folder=None, scenario_file=None, location_file=None, output_file=None, domain_file=None):
   ...
   # If location not specified use default
    ...
    order_train = problem.add_fluent(up.Fluent("order_train", up.IntType(0), train=arrival_train_type), default_initial_value=up.Int(0))
    ...
    # Relative order follows the thesis model: 1 is the A-side end and the
    # current track count is the B-side end. Inactive units keep order 0.
    order_su         = problem.add_fluent(up.Fluent("order_su", up.IntType(0), shunting_unit=shunting_unit_type), default_initial_value=up.Int(0))
    direction_su     = problem.add_fluent(up.Fluent("direction_su", up.BoolType(), shunting_unit=shunting_unit_type), default_initial_value=False)
    ...
    def shift_su_orders(action, track, condition, delta):
        """Shift every active shunting unit on a track that satisfies condition."""
        other = up.Variable(f"{action.name}_ordered_su", shunting_unit_type)
        action.add_effect(
            order_su(other),
            order_su(other) + delta,
            condition=up.And(active_su(other), at_su(other, track), condition(other)),
            forall=[other],
        )

    def shift_train_orders(action, track, condition, delta):
        """Arrival-layer equivalent retained for compatibility when enabled."""
        other = up.Variable(f"{action.name}_ordered_train", arrival_train_type)
        action.add_effect(
            order_train(other),
            order_train(other) + delta,
            condition=up.And(at(other, track), condition(other)),
            forall=[other],
        )
    ...
    move_aside_empty.add_precondition(up.Equals(order_train(move_aside_empty.t), 1))
    move_aside_empty.add_precondition(up.Equals(number_of_trains_on_track(move_aside_empty.l_to), 0))
    move_aside_empty.add_precondition(occupied_length(move_aside_empty.l_to) + train_length(move_aside_empty.t) <= track_length(move_aside_empty.l_to))
    ...
    move_aside_empty.add_effect(occupied_length(move_aside_empty.l_from), occupied_length(move_aside_empty.l_from) - train_length(move_aside_empty.t))
    move_aside_empty.add_effect(occupied_length(move_aside_empty.l_to), occupied_length(move_aside_empty.l_to) + train_length(move_aside_empty.t))
    shift_train_orders(move_aside_empty, move_aside_empty.l_from, lambda other: order_train(other) > order_train(move_aside_empty.t), -1)
    move_aside_empty.add_effect(order_train(move_aside_empty.t), number_of_trains_on_track(move_aside_empty.l_to) + 1)
    ...
    move_aside_empty_su.add_precondition(direction_su(move_aside_empty_su.su))
    ...
    move_aside_empty_su.add_precondition(up.Equals(order_su(move_aside_empty_su.su), 1))
    ...
    shift_su_orders(move_aside_empty_su, move_aside_empty_su.l_from, lambda other: order_su(other) > order_su(move_aside_empty_su.su), -1)
    move_aside_empty_su.add_effect(order_su(move_aside_empty_su.su), number_of_trains_on_track(move_aside_empty_su.l_to) + 1)
    ...
    move_aside_occupied.add_precondition(up.Equals(order_train(move_aside_occupied.t), 1))
    move_aside_occupied.add_precondition(number_of_trains_on_track(move_aside_occupied.l_to) > 0)
    move_aside_occupied.add_precondition(occupied_length(move_aside_occupied.l_to) + train_length(move_aside_occupied.t) <= track_length(move_aside_occupied.l_to))
    ...
    move_aside_occupied.add_effect(occupied_length(move_aside_occupied.l_from), occupied_length(move_aside_occupied.l_from) - train_length(move_aside_occupied.t))
    move_aside_occupied.add_effect(occupied_length(move_aside_occupied.l_to), occupied_length(move_aside_occupied.l_to) + train_length(move_aside_occupied.t))
    shift_train_orders(move_aside_occupied, move_aside_occupied.l_from, lambda other: order_train(other) > order_train(move_aside_occupied.t), -1)
    move_aside_occupied.add_effect(order_train(move_aside_occupied.t), number_of_trains_on_track(move_aside_occupied.l_to) + 1)
    ...
    move_aside_occupied_su.add_precondition(direction_su(move_aside_occupied_su.su))
    ...
    move_aside_occupied_su.add_precondition(up.Equals(order_su(move_aside_occupied_su.su), 1))
    ...
    shift_su_orders(move_aside_occupied_su, move_aside_occupied_su.l_from, lambda other: order_su(other) > order_su(move_aside_occupied_su.su), -1)
    move_aside_occupied_su.add_effect(order_su(move_aside_occupied_su.su), number_of_trains_on_track(move_aside_occupied_su.l_to) + 1)
    ...
    move_bside_empty.add_precondition(up.Equals(order_train(move_bside_empty.t), number_of_trains_on_track(move_bside_empty.l_from)))
    move_bside_empty.add_precondition(up.Equals(number_of_trains_on_track(move_bside_empty.l_to), 0))
    move_bside_empty.add_precondition(occupied_length(move_bside_empty.l_to) + train_length(move_bside_empty.t) <= track_length(move_bside_empty.l_to))
    ...
    move_bside_empty.add_effect(occupied_length(move_bside_empty.l_from), occupied_length(move_bside_empty.l_from) - train_length(move_bside_empty.t))
    move_bside_empty.add_effect(occupied_length(move_bside_empty.l_to), occupied_length(move_bside_empty.l_to) + train_length(move_bside_empty.t))
    shift_train_orders(move_bside_empty, move_bside_empty.l_to, lambda other: order_train(other) >= 1, 1)
    move_bside_empty.add_effect(order_train(move_bside_empty.t), 1)
    ...
    move_bside_empty_su.add_precondition(up.Not(direction_su(move_bside_empty_su.su)))
    ...
    move_bside_empty_su.add_precondition(up.Equals(order_su(move_bside_empty_su.su), number_of_trains_on_track(move_bside_empty_su.l_from)))
    ...
    shift_su_orders(move_bside_empty_su, move_bside_empty_su.l_to, lambda other: order_su(other) >= 1, 1)
    move_bside_empty_su.add_effect(order_su(move_bside_empty_su.su), 1)
    ...
    move_bside_occupied.add_precondition(up.Equals(order_train(move_bside_occupied.t), number_of_trains_on_track(move_bside_occupied.l_from)))
    move_bside_occupied.add_precondition(number_of_trains_on_track(move_bside_occupied.l_to) > 0)
    move_bside_occupied.add_precondition(occupied_length(move_bside_occupied.l_to) + train_length(move_bside_occupied.t) <= track_length(move_bside_occupied.l_to))
    ...
    move_bside_occupied.add_effect(occupied_length(move_bside_occupied.l_from), occupied_length(move_bside_occupied.l_from) - train_length(move_bside_occupied.t))
    move_bside_occupied.add_effect(occupied_length(move_bside_occupied.l_to), occupied_length(move_bside_occupied.l_to) + train_length(move_bside_occupied.t))
    shift_train_orders(move_bside_occupied, move_bside_occupied.l_to, lambda other: order_train(other) >= 1, 1)
    move_bside_occupied.add_effect(order_train(move_bside_occupied.t), 1)
    ...
    move_bside_occupied_su.add_precondition(up.Not(direction_su(move_bside_occupied_su.su)))
    ...
    move_bside_occupied_su.add_precondition(up.Equals(order_su(move_bside_occupied_su.su), number_of_trains_on_track(move_bside_occupied_su.l_from)))
    ...
    shift_su_orders(move_bside_occupied_su, move_bside_occupied_su.l_to, lambda other: order_su(other) >= 1, 1)
    move_bside_occupied_su.add_effect(order_su(move_bside_occupied_su.su), 1)
    ...
    depart_aside.add_precondition(up.Equals(order_train(depart_aside.t), 1))
    ...
    depart_aside.add_effect(occupied_length(depart_aside.l), occupied_length(depart_aside.l) - train_length(depart_aside.t))
    shift_train_orders(depart_aside, depart_aside.l, lambda other: order_train(other) > order_train(depart_aside.t), -1)
    depart_aside.add_effect(order_train(depart_aside.t), 0)
    ...
    depart_bside.add_precondition(up.Equals(order_train(depart_bside.t), number_of_trains_on_track(depart_bside.l)))
    ...
    depart_bside.add_effect(occupied_length(depart_bside.l), occupied_length(depart_bside.l) - train_length(depart_bside.t))
    depart_bside.add_effect(order_train(depart_bside.t), 0)
    ...
    depart_aside_su.add_precondition(direction_su(depart_aside_su.su))
    ...
    depart_aside_su.add_precondition(up.Equals(order_su(depart_aside_su.su), 1))
    ...
    shift_su_orders(depart_aside_su, depart_aside_su.l, lambda other: order_su(other) > order_su(depart_aside_su.su), -1)
    depart_aside_su.add_effect(order_su(depart_aside_su.su), 0)
    ...
    depart_bside_su.add_precondition(up.Not(direction_su(depart_bside_su.su)))
    ...
    depart_bside_su.add_precondition(up.Equals(order_su(depart_bside_su.su), number_of_trains_on_track(depart_bside_su.l)))
    ...
    depart_bside_su.add_effect(order_su(depart_bside_su.su), 0)
    ...
    depart_aside_su_for_request.add_precondition(direction_su(depart_aside_su_for_request.su))
    ...
    depart_aside_su_for_request.add_precondition(up.Equals(order_su(depart_aside_su_for_request.su), 1))
    ...
    shift_su_orders(depart_aside_su_for_request, depart_aside_su_for_request.l, lambda other: order_su(other) > order_su(depart_aside_su_for_request.su), -1)
    depart_aside_su_for_request.add_effect(order_su(depart_aside_su_for_request.su), 0)
    ...
    depart_bside_su_for_request.add_precondition(up.Not(direction_su(depart_bside_su_for_request.su)))
    ...
    depart_bside_su_for_request.add_precondition(up.Equals(order_su(depart_bside_su_for_request.su), number_of_trains_on_track(depart_bside_su_for_request.l)))
    ...
    depart_bside_su_for_request.add_effect(order_su(depart_bside_su_for_request.su), 0)
    ...
    # Shunting units retain the baseline's discrete facing semantics while their
    # metre-level position is replaced by frontmost/adjacency relations.
    turn_a_side_su = up.InstantaneousAction('turn_a_side_su', su=shunting_unit_type, l=track_part_type)
    turn_a_side_su.add_precondition(active_su(turn_a_side_su.su))
    turn_a_side_su.add_precondition(allowed_to_move_su(turn_a_side_su.su))
    turn_a_side_su.add_precondition(up.Not(direction_su(turn_a_side_su.su)))
    turn_a_side_su.add_precondition(at_su(turn_a_side_su.su, turn_a_side_su.l))
    turn_a_side_su.add_precondition(turning_allowed(turn_a_side_su.l))
    turn_a_side_su.add_effect(direction_su(turn_a_side_su.su), True)
    problem.add_action(turn_a_side_su)

    turn_b_side_su = up.InstantaneousAction('turn_b_side_su', su=shunting_unit_type, l=track_part_type)
    turn_b_side_su.add_precondition(active_su(turn_b_side_su.su))
    turn_b_side_su.add_precondition(allowed_to_move_su(turn_b_side_su.su))
    turn_b_side_su.add_precondition(direction_su(turn_b_side_su.su))
    turn_b_side_su.add_precondition(at_su(turn_b_side_su.su, turn_b_side_su.l))
    turn_b_side_su.add_precondition(turning_allowed(turn_b_side_su.l))
    turn_b_side_su.add_effect(direction_su(turn_b_side_su.su), False)
    problem.add_action(turn_b_side_su)
    ...
    # Replace order k with children k and k+1; following units shift by one.
    shift_su_orders(split_two_unit_su, split_two_unit_su.track, lambda other: order_su(other) > order_su(split_two_unit_su.parent_su), 1)
    split_two_unit_su.add_effect(order_su(split_two_unit_su.left_su), order_su(split_two_unit_su.parent_su))
    split_two_unit_su.add_effect(order_su(split_two_unit_su.right_su), order_su(split_two_unit_su.parent_su) + 1)
    split_two_unit_su.add_effect(order_su(split_two_unit_su.parent_su), 0)
    split_two_unit_su.add_effect(direction_su(split_two_unit_su.left_su), True, condition=direction_su(split_two_unit_su.parent_su))
    split_two_unit_su.add_effect(direction_su(split_two_unit_su.right_su), True, condition=direction_su(split_two_unit_su.parent_su))
    ...
    # Replace order k with k, k+1, k+2; following units shift by two.
    shift_su_orders(split_three_unit_su, split_three_unit_su.track, lambda other: order_su(other) > order_su(split_three_unit_su.parent_su), 2)
    split_three_unit_su.add_effect(order_su(split_three_unit_su.first_su), order_su(split_three_unit_su.parent_su))
    split_three_unit_su.add_effect(order_su(split_three_unit_su.second_su), order_su(split_three_unit_su.parent_su) + 1)
    split_three_unit_su.add_effect(order_su(split_three_unit_su.third_su), order_su(split_three_unit_su.parent_su) + 2)
    split_three_unit_su.add_effect(order_su(split_three_unit_su.parent_su), 0)
    for child_su in (split_three_unit_su.first_su, split_three_unit_su.second_su, split_three_unit_su.third_su):
        split_three_unit_su.add_effect(direction_su(child_su), True, condition=direction_su(split_three_unit_su.parent_su))
    ...
    # Coupling always requires physical adjacency. The optional order choice determines
    # whether the request's slot order must also match A-to-B track order.
    if ENFORCE_COUPLING_ORDER:
        couple_two_sus.add_precondition(up.Equals(order_su(couple_two_sus.su_b), order_su(couple_two_sus.su_a) + 1))
    else:
        couple_two_sus.add_precondition(up.Or(
            up.Equals(order_su(couple_two_sus.su_b), order_su(couple_two_sus.su_a) + 1),
            up.Equals(order_su(couple_two_sus.su_a), order_su(couple_two_sus.su_b) + 1),
        ))
    ...
    # The result takes the lower order; units behind the pair shift down by one.
    couple_two_sus.add_effect(order_su(couple_two_sus.su_result), order_su(couple_two_sus.su_a), condition=order_su(couple_two_sus.su_a) < order_su(couple_two_sus.su_b))
    couple_two_sus.add_effect(order_su(couple_two_sus.su_result), order_su(couple_two_sus.su_b), condition=order_su(couple_two_sus.su_b) < order_su(couple_two_sus.su_a))
    coupling_other = up.Variable("coupling_ordered_su", shunting_unit_type)
    coupling_after_pair = up.Or(
        up.And(order_su(couple_two_sus.su_a) < order_su(couple_two_sus.su_b), order_su(coupling_other) > order_su(couple_two_sus.su_b)),
        up.And(order_su(couple_two_sus.su_b) < order_su(couple_two_sus.su_a), order_su(coupling_other) > order_su(couple_two_sus.su_a)),
    )
    couple_two_sus.add_effect(
        order_su(coupling_other),
        order_su(coupling_other) - 1,
        condition=up.And(active_su(coupling_other), at_su(coupling_other, couple_two_sus.track), coupling_after_pair),
        forall=[coupling_other],
    )
    couple_two_sus.add_effect(order_su(couple_two_sus.su_a), 0)
    couple_two_sus.add_effect(order_su(couple_two_sus.su_b), 0)
    couple_two_sus.add_effect(direction_su(couple_two_sus.su_result), True, condition=direction_su(couple_two_sus.su_a))
    ...
    corridor_nodes = _relevant_corridor_nodes(
        scenario_object,
        location_object,
        id_to_track_part.keys(),
        coupling_candidate_track_ids,
        service_track_ids.keys() if ENABLE_SERVICING else (),
        expand_hops=CORRIDOR_EXPAND_HOPS,
    ) if ENABLE_CORRIDOR else None
    ...
    for index, train in enumerate(inbound_trains):
        ...
        track_train_order.setdefault(initial_track_id, []).append(arrival_train)
    ...
    for index, train in enumerate(in_standing_trains):
        ...
        if initial_track_id is not None:
            ...
            track_train_order.setdefault(initial_track_id, []).append(standing_train)
            track_occupancies[initial_track_id] = track_occupancies.get(initial_track_id, Fraction(0)) + train_total_length
            track_train_counts[initial_track_id] = track_train_counts.get(initial_track_id, 0) + 1
    ...
    problem.add_goal(up.Equals(concurrent_movements, up.Int(0)))
    ...
    for source, index, train in all_trains_with_source(scenario_object):
        ...
        if physical_train is None:
            ...
            if initial_track_id in id_to_track_part:
                ...
                track_train_order.setdefault(initial_track_id, []).append(physical_train)
        ...
        if initial_track_id in id_to_track_part:
            ...
            track_su_order.setdefault(initial_track_id, []).append(shunting_unit)
            problem.set_initial_value(direction_su(shunting_unit), determine_initial_direction(train, location_object, initial_track_id))
    ...
    # Scenario insertion order is interpreted from A-side to B-side.
    for track_id, ordered_sus in track_su_order.items():
        for position, shunting_unit in enumerate(ordered_sus, start=1):
            problem.set_initial_value(order_su(shunting_unit), up.Int(position))

    for track_id, ordered_trains in track_train_order.items():
        for position, train in enumerate(ordered_trains, start=1):
            problem.set_initial_value(order_train(train), up.Int(position))
