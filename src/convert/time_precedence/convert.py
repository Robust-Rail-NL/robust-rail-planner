...
def create_instance_from_scenario(path_to_folder=None, scenario_file=None, location_file=None, output_file=None, domain_file=None, coupling_mode="implicit_free_uncoupling", subproblem="combined"):
   ...
   # If location not specified use default
    ...
    if explicit_uncoupling:
        ...
        uncouple.add_increase_effect(total_cost(), 120)
    ...
    if include_parking:
        ...
        # Build arrival-precedence chain for inbound trains (not inStanding — those are already present).
        # Sort by scheduled arrival time; the earliest train gets previous_arrived=True immediately.
        # Each consecutive pair (earlier, later) gets arrival_immediately_before(earlier, later)=True,
        # so that when earlier arrives, the arrive action's conditional effect unlocks later.
        inbound_sorted = sorted(inbound_trains, key=lambda t: int(t.get("arrival", 0)))
        if inbound_sorted:
            first_idx = inbound_trains.index(inbound_sorted[0])
            first_obj = train_obj_by_key[("in", first_idx)]
            problem.set_initial_value(previous_arrived(first_obj), True)
        for earlier, later in zip(inbound_sorted, inbound_sorted[1:]):
            earlier_obj = train_obj_by_key[("in", inbound_trains.index(earlier))]
            later_obj   = train_obj_by_key[("in", inbound_trains.index(later))]
            problem.set_initial_value(arrival_immediately_before(earlier_obj, later_obj), True)
