...
def create_instance_from_scenario(path_to_folder=None, scenario_file=None, location_file=None, output_file=None, domain_file=None):
   ...
   # If location not specified use default
    ...
    total_cost = problem.add_fluent(up.Fluent("total-cost", up.IntType()), default_initial_value=up.Int(0))
    ...
    previous_arrived = problem.add_fluent(
        up.Fluent("previous_arrived", up.BoolType(), train=arrival_train_type),
        default_initial_value=False)
    arrival_immediately_before = problem.add_fluent(
        up.Fluent("arrival_immediately_before", up.BoolType(),
                  first=arrival_train_type, second=arrival_train_type),
        default_initial_value=False)
    ...
    move_aside_empty.add_increase_effect(total_cost(), 300)
    ...
    move_aside_occupied.add_increase_effect(total_cost(), 300)
    ...
    move_bside_empty.add_increase_effect(total_cost(), 300)
    ...
    move_bside_occupied.add_increase_effect(total_cost(), 300)
    ...
    # once its precedence guard has been satisfied (see previous_arrived below).
    ...
    arrive.add_precondition(previous_arrived(arrive.t))
    ...
    next_t = up.Variable("next_t", arrival_train_type)
    arrive.add_effect(
        fluent=previous_arrived(next_t),
        value=True,
        condition=arrival_immediately_before(arrive.t, next_t),
        forall=[next_t],
    )
    problem.add_action(arrive)

    part_of_composition = problem.add_fluent(
        up.Fluent("part_of_composition", up.BoolType(), unit=train_unit_type, composition=arrival_composition_type),
        default_initial_value=False)
    composition_needs_uncoupling = problem.add_fluent(
        up.Fluent("composition_needs_uncoupling", up.BoolType(), composition=arrival_composition_type),
        default_initial_value=False)
    ...
    couple_two_units.add_increase_effect(total_cost(), 180)
    ...
    couple_two_units_same_train.add_increase_effect(total_cost(), 180)
    ...
    # Add track part objects (all track parts including switches are kept as explicit objects)
    id_to_track_part = {}
    ...
    # Add inbound trains — NOT pre-placed; they enter the yard via the arrive action.
    # Placing them in the initial state would cause arrive to double-count track occupancy.
    for index, train in enumerate(inbound_trains):
        ...
        train_initial_aside[_train_object_name("in", index, train)] = Fraction(0)

    # Arrival precedence chain: trains arrive in scheduled order.
    # The first train's previous_arrived starts True; each arrival unlocks the next.
    sorted_inbound = sorted(enumerate(inbound_trains), key=lambda p: int(p[1]["arrival"]))
    if sorted_inbound:
        problem.set_initial_value(previous_arrived(train_obj_by_key[("in", sorted_inbound[0][0])]), True)
        for (idx_a, _), (idx_b, _) in zip(sorted_inbound, sorted_inbound[1:]):
            problem.set_initial_value(
                arrival_immediately_before(train_obj_by_key[("in", idx_a)], train_obj_by_key[("in", idx_b)]),
                True
            )
