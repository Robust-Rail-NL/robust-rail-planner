...
# When True, keep only the edges lying on each relevant route (plus maneuver edges to
# dead-end/coupling neighbours) instead of every edge between corridor nodes. Far fewer
# transitions. Replaces the node-level hop expansion when on. Only used with ENABLE_CORRIDOR.
CORRIDOR_EDGE_LEVEL = False
...
parser.add_argument("--corridor-edges", action=argparse.BooleanOptionalAction, default=CORRIDOR_EDGE_LEVEL,
                    help="Edge-level corridor: keep only route edges plus maneuver edges, not every edge between corridor nodes. Default: %(default)s")
...
def _relevant_corridor_edges(scenario_object, location_object, known_track_ids, coupling_candidate_track_ids):
    # Edge-level corridor: keep only the edges lying on each relevant route, plus maneuver
    # edges from route nodes to their dead-end (degree 1) and coupling-allowed neighbours so
    # reversals onto buffer/siding tracks stay possible. Returns a set of frozenset({a, b})
    # of str ids, or None to mean "do not restrict".
    raw_adj = _build_adjacency(location_object)
    adjacency = {str(k): {str(n) for n in v} for k, v in raw_adj.items()}
    edges = set()
    route_nodes = set()
    ...
    coupling_set = {str(c) for c in coupling_candidate_track_ids}
    for node in route_nodes:
        for nb in adjacency.get(node, ()):
            if len(adjacency.get(nb, ())) <= 1 or nb in coupling_set:
...
def create_instance_from_scenario(path_to_folder=None, scenario_file=None, location_file=None, output_file=None, domain_file=None):
   ...
   # If location not specified use default
    ...
    if ENABLE_CORRIDOR and CORRIDOR_EDGE_LEVEL:
        corridor_edges = _relevant_corridor_edges(scenario_object, location_object, id_to_track_part.keys(), coupling_candidate_track_ids)
        corridor_nodes = None
    elif ENABLE_CORRIDOR:
    ...
    def _in_corridor(*ids):
        if corridor_edges is not None:
            return len(ids) != 2 or frozenset((str(ids[0]), str(ids[1]))) in corridor_edges
...
if __name__ == "__main__":
    ...
    CORRIDOR_EDGE_LEVEL = args.corridor_edges
