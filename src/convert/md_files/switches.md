...
| v0.5 | 2026-05-31 | Current routing-only switch variant: parking plus turn-aware routing |
...
## Branch Additions
- `convert_switches.py` emits the parking/routing-only domain variant with explicit turning support.
- The model uses `start_move`, `end_move`, `move_aside_*`, `move_bside_*`, `turn_a_side`, `turn_b_side`, `depart_aside`, `depart_bside`, and `park`.
...
| `turning_allowed` | `(trackpart)` | Bool | Whether a track part allows turning | v0.5 |
...
| `num_of_departed_trains` | `()` | Int | Counter for departed trains | v0.3 |
...
| `direction_train` | `(arrivaltrain)` | Bool | True means the train faces aside, false means bside | v0.5 |
...
### `turn_a_side`
- **Parameters:** `t - arrivaltrain`, `l - trackpart`
- **Description:** Turns a train to face the aside direction on a turnable track part.

### `turn_b_side`
- **Parameters:** `t - arrivaltrain`, `l - trackpart`
- **Description:** Turns a train to face the bside direction on a turnable track part.
...
| `turning_allowed(track)` | `location_solver.json -> trackParts[].sawMovementAllowed` | Set for turnable track parts |
...
| `direction_train(train)` | Initial orientation heuristic | Set from the initial track and whether the track is turnable |
