...
def _resolve_departure_track_sides(ids, location_object):
        ...
        while queue:
            ...
            for nb in adjacency.get(current, []):
                ...
                if nb_str in visited:
                    continue
                visited.add(nb_str)
                nb_tp = track_parts.get(nb_str)
                if nb_tp is not None and _is_switch_like_track_part(nb_tp):
                    queue.append(nb_str)
                else:
                    real_hits.add((nb_str, current))
        if not real_hits:
            real_hits = {(str_id, str_id)}
        for real_id, came_from in real_hits:
...
def _resolve_departure_track_ids(ids, location_object):
    # Convenience wrapper returning just the resolved real track ids.
    return set(_resolve_departure_track_sides(ids, location_object))
...
def _departure_exit_ids(scenario_object, location_object):
    ...
    # Falls back to the inbound first parking tracks if no outbound requests are present in the scenario.
    ...
    sides_by_track = _resolve_departure_track_sides(ids, location_object)

    ids_aside = {tid for tid, sides in sides_by_track.items() if "a" in sides}
    ids_bside = {tid for tid, sides in sides_by_track.items() if "b" in sides}
...
def _relevant_corridor_nodes(scenario_object, location_object,
    ...
    for request in scenario_object.get("out", []):
        ...
        leave_ids = _resolve_departure_track_ids(
...
def _is_switch_like_track_part(track_part):
    ...
    # Bumpers (dead-end buffer/signal nodes like Stootblok/Sein) are removed the same way:
    # they are zero-length, never hold a train, and only provide an exit boundary.
    ...
    try:
        ...
        length = Fraction(0)
    return length == 0 and (
        len(_track_part_neighbors(track_part)) >= 2 or track_part.get("type") == "Bumper"
    )
...
def create_instance_from_scenario(
    ...
    # arrive_su.add_effect(allowed_to_move_su(arrive_su.su), True) # remove version 1
    # arrive_su.add_effect(concurrent_movements, concurrent_movements + 1) # remove version 2
    ...
    park_su = up.InstantaneousAction('park_su', su=shunting_unit_type, unit=train_unit_type, slot=parking_slot_type, l=track_part_type)
    ...
    park_su.add_precondition(contains_su(park_su.su, park_su.unit))
    park_su.add_precondition(parking_slot_track(park_su.slot, park_su.l))
    park_su.add_precondition(parking_compatible(park_su.unit, park_su.slot))
    park_su.add_precondition(up.Not(parked_unit_used(park_su.unit)))
    ...
    park_su.add_effect(parking_slot_fulfilled(park_su.slot), True)
    park_su.add_effect(parked_unit_used(park_su.unit), True)
    ...
    service_su.add_precondition(up.Not(allowed_to_move_su(service_su.su))) # add version 3
    ...
    if compile_precomputed_actions:
        ...
        def add_compiled_uncouple(name, front):
            ...
            action.add_precondition(up.Not(allowed_to_move_su(action.parent_su))) # add version 3
            # action.add_precondition(allowed_to_move_su(action.parent_su)) # remove version 2
            action.add_precondition(su_may_move(action.parent_su)) # add version 2
    ...
    if corridor_nodes is not None:
        ...
        for train in in_standing_trains:
            tid = _train_initial_track_id(train, ["firstParkingTrackPart"])
            if tid is not None:
    ...
    for index, train in enumerate(in_standing_trains):
        initial_track_id = _train_initial_track_id(train, ["firstParkingTrackPart"])
    ...
    # must park there, which also marks the unit used and the slot fulfilled.
    for request in out_standing_trains:
    ...
    if compile_precomputed_actions:
        ...
        if ordered_arrival_sus and all(su in departure_su_by_source for su in ordered_arrival_sus):
            ...
            # for su in ordered_arrival_sus: # remove version 1
            #     problem.set_initial_value(su_previous_arrived(su), True) # remove version 1
