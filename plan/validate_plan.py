...
def _precondition_status(evaluator, state, grounded_precondition):
    try:
        value = evaluator.evaluate(grounded_precondition, state)
        return True, value
    except Exception as exc:
        return False, exc


def _trace_condition(evaluator, state, expr, indent="  "):
    if expr.is_and():
        print(f"{indent}AND {expr}")
        for child_idx, child in enumerate(expr.args, start=1):
            child_ok, child_result = _trace_condition(evaluator, state, child, indent + "  ")
            if not child_ok:
                return False, child_result
        return True, None

    ok, result = _precondition_status(evaluator, state, expr)
    if ok:
        try:
    ...
    for idx, action_instance in enumerate(plan.actions, start=1):
        ...
        if not applicable:
            ...
            for pre_idx, precondition in enumerate(action.preconditions, start=1):
                ...
                print(f"  {pre_idx}.")
                ok, result = _trace_condition(evaluator, state, grounded, indent="    ")
